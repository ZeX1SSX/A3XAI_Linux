...
while {(count A3XAI_respawnQueue) > 0} do {
	...
	//Begin examining queue entries.
	{
		...
		if (diag_tickTime > _timeToRespawn) then {
			...
			if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Comparing new respawn time %1 with previous %2.",_delay,_minDelay];};
			if (_minDelay > 0) then {
				//If next delay time is smaller than the current minimum delay, use it instead.
				if (_delay < _minDelay) then {
					_minDelay = _delay;
					if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Found shorter respawn interval: %1 seconds.",_minDelay];};
				};
			} else {
				//Initialize minimum delay to first found delay.
				_minDelay = _delay;
				if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Set respawn interval to %1 seconds.",_minDelay];};
			};
		};
	} forEach A3XAI_respawnQueue;
	if (_minDelay > -1) then {
