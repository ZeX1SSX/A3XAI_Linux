...
if !(_unitLevel in A3XAI_unitLevelsAll) then {
	...
	if (A3XAI_debugLevel > 1) then {diag_log format ["A3XAI Error: Invalid unitLevel provided: %1. Generating new unitLevel value: %2. (%3)",_unitLevelInvalid,_unitLevel,__FILE__];};
};
...
if !(_uniforms isEqualTo []) then {
	...
	if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG: %1",_uniformItem];};
};
...
if (_backpackChance call A3XAI_chance) then {
	...
	if !(_backpacks isEqualTo []) then {
		...
		if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG: %1",_backpackItem];};
	};
};
...
if (_vestChance call A3XAI_chance) then {
	...
	if !(_vests isEqualTo []) then {
		...
		if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG: %1",_vestItem];};
	};
};
...
if ((getNumber (configFile >> "CfgMagazines" >> _magazine >> "count")) < 6) then {
	...
	if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG: %1",_magazine];};
};
...
if ((missionNamespace getVariable [("A3XAI_opticsChance"+_unitLevelString),3]) call A3XAI_chance) then {
	...
	if !(_opticsList isEqualTo []) then {
		...
		if (_opticsType in _opticsList) then {
			...
			if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Added optics item %1 to unit %2.",_opticsType,_unit];};
		};
	};
};
...
if ((missionNamespace getVariable [("A3XAI_pointerChance"+_unitLevelString),3]) call A3XAI_chance) then {
	...
	if !(_pointersList isEqualTo []) then {
		...
		if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Added pointer item %1 to unit %2.",_pointerType,_unit];};
	};
};
...
if ((missionNamespace getVariable [("A3XAI_muzzleChance"+_unitLevelString),3]) call A3XAI_chance) then {
	...
	if !(_muzzlesList isEqualTo []) then {
		...
		if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Added muzzle item %1 to unit %2.",_muzzleType,_unit];};
	};
};
...
if ((missionNamespace getVariable [("A3XAI_underbarrelChance"+_unitLevelString),3]) call A3XAI_chance) then {
	...
	if !(_underbarrelList isEqualTo []) then {
		...
		if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Added underbarrel item %1 to unit %2.",_underbarrelType,_unit];};
	};
};
