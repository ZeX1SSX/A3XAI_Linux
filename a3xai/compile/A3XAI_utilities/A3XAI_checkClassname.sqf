...
if (_configIndex > -1) then {
	...
	if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Classname check result: %1. ClassnameArray: %2.",_result,_classnameArray];};
};

if (A3XAI_debugLevel > 1) then {diag_log format ["DEBUG :: Classname %1 (check result: %2) completed in %3 seconds.",_classname,_result,diag_tickTime - _startTime];};
