...
if (A3XAI_generateDynamicUniforms) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildUniform", false]) == true;};
};
...
if (A3XAI_generateDynamicWeapons) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildWeapon", false]) == true;};
};
...
if (A3XAI_generateDynamicBackpacks) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildBackpack", false]) == true;};
};
...
if (A3XAI_generateDynamicVests) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildVest", false]) == true;};
};
...
if (A3XAI_generateDynamicHeadgear) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildHeadgear", false]) == true;};
};
...
if (A3XAI_generateDynamicFood) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildFood", false]) == true;};
};
...
if (A3XAI_generateDynamicLoot) then {
	...
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildLoot", false]) == true;};
};

if (A3XAI_generateDynamicOptics) then {
	_weaponScopes = [] execVM format ['%1\scripts\A3XAI_buildScopeList.sqf',A3XAI_directory];
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneBuildScope", false]) == true;};
};

//Check classname tables if enabled
if (A3XAI_verifyClassnames) then {
	_verifyClassnames = [] execVM format ["%1\scripts\verifyClassnames.sqf",A3XAI_directory];
	waitUntil {uiSleep 0.05; (missionNamespace getVariable ["_doneVerifyClassnames", false]) == true;};
};
...
if (A3XAI_loadCustomFile) then {
	...
	if (A3XAI_debugLevel > 1) then {diag_log "A3XAI Debug: No custom spawns";};
