...
private ["_playerUID_DB", "_lastSpawned_DB", "_lastOnline_DB"];
...
while {true} do {
	...
	if ((count (allPlayers select {isPlayer _x && {(getPlayerUID _x select [0,2]) != "HC"}})) > 0)
	// if (({alive _x} count allPlayers) > 0) 
	then {
		...
		_currentTime = diag_tickTime;
		{
			if ((isPlayer _x) && {((typeOf _x) in [PLAYER_UNITS])}) then {
				...
				if !((_playerUID select [0,2]) isEqualTo "HC") then {
					...
					if (_playerIndex > -1) then {
						...
						if (_timePassed > A3XAI_timePerDynamicSpawn) then {
							if ((_currentTime - (_lastOnline_DB select _playerIndex)) < A3XAI_purgeLastDynamicSpawnTime) then {
								...
								if (A3XAI_debugLevel > 1) then {diag_log format ["A3XAI Debug: Player %1 added to current cycle dynamic spawn list.",_x];};
							};
					...
					if (A3XAI_debugLevel > 2) then {diag_log format ["A3XAI Debug: Found a player at %1 (%2).",mapGridPosition _x,name _x];};
				};
			};
			uiSleep 0.05;
		} forEach allPlayers;
